...
How to handle cases that fail to begin and end near the first and last stop of a route, respectively, could have a significant effect on the amount of data available to analyze. Filtering only to rawnav run records that begin with 500 feet of the first stop and end within 500 feet of the last stop keeps only about 60 percent of run records. This rate could potentially be increased with some additional logic added to the in-development rawnav parser.
...
summary79 <-
  ...
  make_extra_geom(StartLong, StartLat,geom_start,4326) %>%
  make_extra_geom(EndLong, EndLat, geom_end, 4326)
...
Southbound, it appears other runs never fully depart, and some may not close out appropriately. It's possible some additional logic to close off the trip when it arrives at the final stop could make these cases usable.
...
What of the cases that fall further than `r usefil` feet away from a start or end point? Because rawnav pings are not loaded in this dataset, only the straightlines from the start to the end are shown. Many of these trips are making the journey - and some are making a much different trip! Here we look just at the Northbound trips, which had fewer close out issues previously. 

**79 Northbound Trips With First/Last Stop Issues** 
```{r}
summary79_4 <-
  summary79_3 %>%
  select(StartLong,StartLat,EndLong,EndLat) %>%
...
summary79_4 %>%
  ...
  mapview(.,
          ...
          homebutton = TRUE)
```
Below, we filter to the shorter of these segments -- a handful of these shorter runs start but never quite get going, while others make it past Columbia Heights before terminating early. 

**79 Northbound Trips With First/Last Stop Issues, Short Trips** 
```{r}
summary79_4 %>%
  filter(dirname == "Northbound") %>%
  filter(dist_to_first > usefil | dist_to_last > usefil) %>%
  filter(length < units::set_units(5000,"m")) %>%
  mapview(.,
          zcol = "refid", 
          layer.name = "Run Info",
          legend = FALSE,
...
### Run Times
If we filter to stops that start and end within a set distance from the expected first and last stop, how many records do we get?
```{r}

summary79_fil <-
  summary79_3 %>%
  filter(dist_to_first <= usefil, dist_to_last <= usefil) %>%
  mutate(runtime = as.numeric(trip_end_time - trip_start_time, units = "mins")) 
...
What's the distribution of runtimes? Below are averages by period, in minutes
```{r}
summary79_fil %>%
...
There are a handful of outliers for runtime, but none especially egregious. Outliers for runtime on the high end could be the consequence of operators failing to close out a trip after arriving at the final stop, such that the runtime is artificially extended.
```{r}
summary79_fil %>%
