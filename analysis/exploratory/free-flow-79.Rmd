...
```
Around `r scales::comma(nrow(runs_pulled))` unique runs of the 79 southbound are included in this datset out of `r (total <- nrow(distinct(rawnav_inventory_filtered_79,filename,route_pattern, tag_date,tag_time, tag_busid)))` runs that are in the October 2019 data: close to `r scales::percent(nrow(runs_pulled) / total)`. We'll investigate what happened to that missing run at some point.
...
make_heat <- function(dfin, val, text){
  ggplot(dfin, aes(wday, tag_starthour)) + 
    ...
    geom_text(aes(label = {{text}}),
              color = "grey80") +
    scale_y_reverse('hour',breaks = 4:21) 
}
