...
print(
    "Run Time Section 1 Import Libraries and Set Global Parameters : {}".format(
        executionTime))
print("*" * 100)
...
wmata_schedule_gdf = (
    gpd.GeoDataFrame(
        wmata_schedule_dat,
        geometry=gpd.points_from_xy(wmata_schedule_dat.stop_lon,
...
for analysis_route in analysis_routes:
    ...
    for analysis_day in analysis_days:
        print('Processing analysis route {} for {}...'.format(analysis_route,
                                                              analysis_day))
        ...
        try:
            ...
            rawnav_qjump_gdf = (
                gpd.GeoDataFrame(
                    rawnav_qjump_dat,
                    geometry=gpd.points_from_xy(rawnav_qjump_dat.long,
        ...
        if type(stop_summary) == type(None):
            print('No data on analysis route {} for {}'.format(analysis_route,
                                                               analysis_day))
            continue
