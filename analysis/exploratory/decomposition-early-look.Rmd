...
# Freeflow Speeds
We'll take a first look at freeflow speeds by segment. These were calculated based on a three-ping average speed, excluding any very high speed values above 50 mph.
```{r message=FALSE, warning=FALSE}
...
# some light claculation of fields and filtering
rawnav_stop_area <-
  rawnav_stop_area_raw %>%
  ...
  ungroup() %>%
  # issues with t his for now
  filter(seg_name_id != "georgia_irving")

```

Across all our segments, how long does a bus spend in each zone, excluding runs that go straight through? 
```{r}
rawnav_stop_area %>%
  group_by(busrun,seg_name_id) %>%
...
What does the data we're looking at look like?
```{r}
rawnav_stop_area %>%
  glimpse()
```

Are there cases of reopening doors? 
```{r}
rawnav_stop_area %>% 
  group_by(busrun,seg_name_id) %>%
  ...
  # to do a little more work though to get a value, however.
...
ggplot(run_openings, aes(x = openings)) + 
  ...
  stat_bin(binwidth=binw, geom="text", colour="white", size=3.5,
           aes(label=..count.., group=seg_name_id), position=position_stack(vjust=0.5)) +
...
Here we replicate the speed by distance chart from earlier notebooks. All run odometers are reset such that the rawnav point nearest a stop comes at 150 ft. We see that some of these runs come to a stop before the 150 ft mark. This could be the result of several things:
...
* Buses are stopping short of the bus zone and then edging forward at slow speeds into the bus zone. 
...
* It's more apparent that some bus runs do not let passengers alight at all and proceed directly along in a straight vertical line. 
* Some runs are dwelling at the stop for a considerable amount of time. Note that runs with stop area travel time above 500 seconds are removed from this dataset.
...
Let's add some of the initial decomposition breakdown for an individual run into these charts. This does not reflect the final decomposition (t_stop2 calculations are still to be made), but help to indicate how these values are shaping up. 
...
singleline
```
This concludes the look at the data so far.
